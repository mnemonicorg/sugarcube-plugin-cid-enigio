import {get, curry, set} from 'lodash/fp';
...
const send = curry((u, p, e, dl) => {
  ...
  const url = `https://${u}:${p}@enigio.com/s/timeStamp/reg?size=${size}&mime=${mime}&fileName=${fileName}&sha256=${sha256}&c=${c}&path=${path}&userId=${userId}`;

  return fetch(url).then(r =>
    r.json().then((t) => set('timestamp', get('v', t), dl)));
});
